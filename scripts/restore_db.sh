...
#
# NOTE (home-assistant): stop the `homeassistant` container before restoring the live DB so the
# recorder isn't holding connections / writing mid-restore. Restart it afterwards.
set -eu
...
PGHOST="${POSTGRES_HOST:-ha-postgres}"
PGPORT="${POSTGRES_PORT:-5432}"
PGDATABASE="${POSTGRES_DB:-homeassistant}"
PGUSER="${POSTGRES_USER:-homeassistant}"
...
if [ -z "$BACKUP_FILE" ]; then
    ...
    ls -lh "${BACKUP_DIR:-/backups}/${SERVICE_NAME:-home-assistant}_db_"*.dump 2>/dev/null || echo "  (none found)"
    exit 1
fi
...
log "Restore complete. Restart the homeassistant container to refresh connections."
